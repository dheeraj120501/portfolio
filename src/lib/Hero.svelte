...
<div class="flex">
  ...
  <!-- Main Section -->
  <div class="flex flex-col items-center sm:items-stretch grow px-4">
    <div
      class="flex flex-col sm:flex-row items-center sm:justify-between sm:items-center mt-16"
    >
      <h1 class="font-bold text-3xl sm:text-6xl">
        Hi, i'm <span class="stress capitalize">{profileData.name}</span>
        <br />
        <span class="stress">web developer.</span>
      </h1>
      <div
        class="w-[200px] h-[200px] sm:w-[400px] sm:h-[400px] rounded-full overflow-clip border-t-4 border-primary-light dark:border-primary-dark border-r-4 my-8 sm:my-0"
      >
    ...
    <div class="sm:text-3xl text-xl">
      Ooga Booga Booga <br />
      Doom Doom
      <span class="stress">Wow</span>.
    </div>

    <div
      class="sm:px-10 py-4 text-center w-full sm:w-fit text-2xl bg-primary-light dark:bg-primary-dark dark:text-black text-white rounded-lg mt-16 cursor-pointer"
      on:click={contact}
    >
      Contact me
    </div>
